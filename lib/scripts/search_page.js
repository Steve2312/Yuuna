var search_results = [];
{
    ...
    let createResultCard = (title, artist, duration, source, beatmap_id, i, y, downloaded) => {
        return(
        `
        <div class="card">
    ...
    var load_nextPage = () => {
        ...
        if (list.scrollTop >= list.scrollHeight - list.offsetHeight && !isProcessing && !lastPage && list.scrollHeight - list.offsetHeight != 0) search_beatmap(false);
    }
    ...
    var search_beatmap = (isNewSearch) => {
        ...
        animate_loading_bar(0.01 * 100, 1000);
        ...
        progress(searchRequest).on('progress', (state) => {
            ...
            try {
                ...
                if (data_length == 0) {
                    ...
                    var allsongs = await getAlldownloaded();
                    for(var i = 0; i < data_length; i++){
                        ...
                        list.innerHTML += createResultCard(title, artist, duration(), source, beatmap_id, i, search_results.length - 1, allsongs.includes(beatmap_id.toString()));
                    }
                }
            } catch (err) {}
        });
