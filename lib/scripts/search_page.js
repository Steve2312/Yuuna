class search_page {
    ...
    #createResultCard(title, artist, duration, source, beatmap_id, i, y, downloaded) {
        ...
        <div class="card">
            ...
            <div class="card_playbutton">
                <p class="play-button"><span onclick='beatmap_dl.add_download_queue(${i}, ${y})'><i ${downloaded ? "style='display:none'" : ""} id="dowload_beatmap_set_${beatmap_id}" class="fas fa-download"></i></span></p>
                <p class="play-button"><span onclick='player.play_preview(${i}, ${y})'><i id="play_searched_data_from_the_search_page_${beatmap_id}" class="fas fa-play"></i></span></p>
            </div>
        </div>
        `);
    }
    ...
    addEventOnSearch() {
        ...
        input.addEventListener("keyup", function() {
            if(that.#inactivityTimeout) window.clearTimeout(that.#inactivityTimeout);
            that.#inactivityTimeout = window.setTimeout(function () {
                if (that.#isProcessing) that.#searchRequest.abort();
                that.search_beatmap(true);
            }, 1000);
        });
    };
    ...
    load_nextPage() {
        ...
        if (list.scrollTop >= list.scrollHeight - list.offsetHeight && !this.#isProcessing && !this.#lastPage && list.scrollHeight - list.offsetHeight != 0) this.search_beatmap(false);
    }

    search_beatmap(isNewSearch) {
        ...
        var buffer = [];
        var buffer_length = 0;
        if (!this.#isProcessing) this.#show_loading_bar();
        this.#page = isNewSearch ? 0 : this.#page + 1;

        this.#searchRequest = request(this.#getLink(query, this.#page, type));
        console.log(this.#getLink(query, this.#page, type))
        this.#isProcessing = true;
        this.#lastPage = false;

        this.#animate_loading_bar(0.01 * 100, 1000);

        progress(this.#searchRequest).on('progress', (state) => {
            ...
            try {
                ...
                this.#search_results.push(data["beatmaps"]);
                if (data_length == 0) {
                    lastPage = true;
                    ...
                    for(var i = 0; i < data_length; i++){
                        var beatmap = this.#search_results[this.#page][i];
                        ...
                        list.innerHTML += this.#createResultCard(title, artist, duration(), source, beatmap_id, i, this.#search_results.length - 1, allsongs.includes(beatmap_id.toString()));
                    }
                }
            } catch (err) {}
        });
    ...
    open_beatmap_site(id){
        shell.openExternal(`https://osu.ppy.sh/beatmapsets/${id}`);
    }

    #animate_loading_bar(width, speed) {
