...

{
    ...
    let copyFiles = (metadata, temp_path) => {
        return new Promise(async function (resolve) {
            for (x in metadata) {
                ...
                var move_dest = path.join(__dirname, "songs", metadata[x]["BeatmapSetID"])
    ...
    var extract_downloaded_file = async (beatmap_id) => {
        ...
        player.recalculateId();
        // Reload All_songs playlist
        if (LOADED_PAGE == "playlist_page") {
            Libraryload_Playlist('All songs');
        }
    }
}
