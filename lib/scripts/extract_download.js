// Global Variables
{
    ...
    let copyFiles = (metadata, temp_path) => {
        return new Promise(async function (resolve) {
            for (var x = 0; x < Object.keys(metadata).length; x++) {
                // Create song folder if there is none
                x = Object.keys(metadata)[x];
                if (!fs.existsSync(path.join(appDataSongs))) {
                    fs.mkdirSync(path.join(appDataSongs));
                }

                if (!fs.existsSync(path.join(appDataSongs, metadata[x].BeatmapSetID))) {
                    fs.mkdirSync(path.join(appDataSongs, metadata[x].BeatmapSetID));
                }
                
                var move_dest = path.join(appDataSongs, metadata[x].BeatmapSetID, metadata[x].BeatmapID)
                if (!fs.existsSync(move_dest)) {
                    fs.mkdirSync(move_dest);
                }
                // Write Metadata file
                fs.writeFile(path.join(move_dest, "metadata.hbk"), JSON.stringify(metadata[x]), 'utf8', function (err) {if (err) {return console.log(err);}});
    ...
    let delete_temp_folder = (beatmap_id) => {
        fs.rmdir(path.join(appDataTemp, beatmap_id.toString()), { recursive: true }, function (){
            console.log("Deleted: " + path.join(appDataTemp, beatmap_id.toString()));
        });
    }

    // Global functions

    var extract_downloaded_file = async (beatmapset_id) => {
        var temp_file = path.join(get_temp_path(), `${beatmapset_id}.zip`);
        var extract_path = path.join(get_temp_path(), beatmapset_id.toString());
        await extract_zip(temp_file, extract_path);
        var filenames = await list_filename(extract_path);
        await copyFiles(await beatmap_metadata(filenames, extract_path), extract_path).then(() => {
            delete_temp_folder(beatmapset_id);
        });
        player.updatePlaylist();

        var files = await get_ids(path.join(appDataSongs, beatmapset_id.toString()));

        function addcard(lib, id) {
            setTimeout(() => playlist_page.updatePlaylist(lib, id), 1000);
        }

        for (var x = 0; x < files.length; x++) {
            addcard("Library", files[x]);
        }
    ...
    // When i change this to prototype use the function from parse.json
    let get_ids = (src) => {
        return new Promise(resolve => {
