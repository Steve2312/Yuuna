...
async function load(page_Playlist, playlist_name){
    ...
    for(i = 0; i < page_playlist_length; i++) {
        ...
        card = playlist_card(cover, title, artist, duration, source, created, playlist_name, beatmapset_id, beatmap_id);
...
function playlist_card(cover, title, artist, duration, source, created, playlist_name, beatmap_set_id, beatmap_id) {
    // <div class="card">
    ...
    div_more.setAttribute("onclick", `show_options("${playlist_name}", "${beatmap_set_id}", "${beatmap_id}")`);
...
function show_options(playlist_uuid, beatmap_set_id, beatmap_id) {
    function display_tooltip(e) {
        document.getElementById("toolbox").innerHTML +=
        `
        <div class="playlist_toolbox" id="playlist_toolbox">
