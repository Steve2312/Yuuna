// Global Variables
{
    ...
    const Player = require('..\\lib\\scripts\\player_old.js');
    ...
    let initPlayer = (songPath) => {
        ...
        audio.on("play", function () {
            ...
            if (preview_button) preview_changeIcon(1);
        });
    }

    let updateTimer = () => {
        if (!onDrag){
            document.getElementById(`player_currenttime`).textContent = format_seconds(audio.seek());
            if (audio.duration() !== 0) {
                document.getElementById(`player_slider`).value = 1000 * audio.seek() / audio.duration();
            }
        }
        document.getElementById(`player_totaltime`).textContent = format_seconds(audio.duration());
    }
    ...
    let updateAudioData = async () => {
        ...
        document.getElementById(`player_artist`).textContent = currentPlaylist[currentId]['artist'];
        document.getElementById(`player_title`).textContent = currentPlaylist[currentId]['title'];
    ...
    var play_preview = (index, page) => {
        ...
        preview_changeIcon(0, id);
        if (preview_src == link) {
            preview_audio.unload();
            preview_src = null;
            preview_changeIcon(1, id);
        } else {
            if(preview_audio) preview_audio.unload();
    ...
    let preview_changeIcon = (i, id) => {
        // 0 = Reset old button and assign new button
        if(i == 0) {
            if (preview_button) preview_button.className  = "fas fa-play";
            preview_button = document.getElementById(id);
        }
        // If user clicks on the pause button
        if (i == 1)  {
            preview_button.className  = "fas fa-play";
            preview_button = null;
        }
        // Set icon to pause
        if (i == 2) preview_button.className  = "fas fa-pause";
    } 
}
