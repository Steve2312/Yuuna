const playlist = function() {
    ...
    playlist.prototype.getuuid = function (data) {
    ...
    playlist.prototype.create = async function() {
        var that = this;
        async function updateFile() {
            console.log("Create")
            var data = await parse_json.read_file(collection);
            data = data ? JSON.parse(data) : {}

            if (queue[0]["type"] == "create") {
                var {name, description, cover} = queue[0];
                var uuid = that.getuuid(data);
                data[uuid] = {
                    name: name,
                ...
                that.write(collection, JSON.stringify(data));
                that.writeImage(path.join(appDataPlaylists, `${uuid}.jpeg`), cover);
            }

            if (queue[0]["type"] == "add") {
                var {playlist_uuid, BeatmapID, BeatmapSetID} = queue[0];
                if (Object.keys(data).includes(playlist_uuid)) {
                    data[playlist_uuid]["songs"].push({BeatmapID, BeatmapSetID})
                    that.write(collection, JSON.stringify(data));
                } else {
    ...
    playlist.prototype.add = async function(playlist_uuid, BeatmapID, BeatmapSetID) {
        queue.push({playlist_uuid, BeatmapID, BeatmapSetID, type: "add"});
        this.create();
    }
    ...
    playlist.prototype.writeImage = function(path, data) {
