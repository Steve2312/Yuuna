const playlist_page = function() {
    ...
    playlist_page.prototype.load = async function(page_Playlist, playlist_name) {
    ...
    playlist_page.prototype.playlist_card = function(cover, title, artist, duration, source, created, playlist_name, beatmap_set_id, beatmap_id) {
        // <div class="card">
    ...
    playlist_page.prototype.toolbox = function(playlist_uuid, beatmap_set_id, beatmap_id) {
        ...
        delete_song.setAttribute("onclick", `toolbox.delete_song("${beatmap_set_id}", "${beatmap_id}")`);
    ...
    playlist_page.prototype.show_options = function(playlist_uuid, beatmap_set_id, beatmap_id) {
        var that = this;
    ...
    playlist_page.prototype.updatePlaylist = async function(playlist_name, beatmap_id) {
        if (playlist_name == name_playlist) {
            var new_playlist = await parse_json.playlist(playlist_name);
            for (var x = 0; x < new_playlist.length; x++) {
                if (beatmap_id == new_playlist[x].BeatmapID) {
                    var {title, artist, source, length, BeatmapSetID, BeatmapID} = new_playlist[x];
                    var duration = utils.format_seconds(length);
                    var song_folder = path.join(appDataSongs, BeatmapSetID, BeatmapID);
                    var created = await this.data_added(song_folder);
                    var cover = path.join(song_folder, "cover.jpg").replace(/\\/g, '/');
                    ...
                    setTimeout(() => card.removeAttribute("style"), 1000)
                }
            }
        }
    }

    playlist_page.prototype.remove_card = function(id) {
        var block = document.getElementById(`card_library_${id}`)
