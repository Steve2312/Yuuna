class playlist_page {
    ...
    async #load(page_Playlist, playlist_name) {
        ...
        var d = document.getElementById("list_playlist_songs");
        if (d) {
            ...
            if (playlist_length == 0) {
                d.innerHTML +=  `<div style="margin: 10px 0px;width: 100%; justify-content: center; display:flex;justify-content: center;"><p style="text-align: center; width: 50%; font-family: NotoBold;margin: 0px 10px;font-size: 10px;color: var(--subject_color);"> This playlist contains no songs. Songs can be downloaded via the "Search" page or can be manually added to any playlist by clicking 3 dots and selecting "Add to playlist". </p></div>`;
            }
            d.append(c);
        }
