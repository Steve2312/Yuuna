const playlist_page = function() {
    ...
    playlist_page.prototype.load = async function(page_Playlist, playlist_name, playlist_data) {
        ...
        if (playlist_data) {
            ...
            DOM_playlist_cover.setAttribute("onclick", `playlist.show_modify_playlist("${playlist_data.uuid}")`)
    ...
    playlist_page.prototype.playlist_card = function(cover, title, artist, duration, source, created, playlist_name, beatmap_set_id, beatmap_id, index) {
        ...
        i_pb.setAttribute("id", `play_${playlist_name.replace(/\s+/g, '_')}_${index ? index : beatmap_id}`);
    ...
    playlist_page.prototype.toolbox = function(playlist_uuid, beatmap_set_id, beatmap_id, index) {
        ...
        add_to_playlist.setAttribute("id", "toolbox_add_to_playlist");
        view_osu.setAttribute("id", "toolbox_view_osu");
        download_beatconnect.setAttribute("id", "toolbox_download_beatconnect");
        delete_song.setAttribute("id", "toolbox_delete_song");
        delete_song_set.setAttribute("id", "toolbox_delete_song_set");
        ...
        if (playlist_uuid != "Library") {
            ...
            remove_from_playlist.setAttribute("onclick", `toolbox.remove_from_playlist("${playlist_uuid}", "${index}")`);
            remove_from_playlist.setAttribute("id", "toolbox_remove_from_playlist");
    ...
    playlist_page.prototype.toolbox_playlists = function(playlist, beatmap_id, beatmap_set_id) {
        var div = document.createElement("DIV");
        div.setAttribute("id", "playlist_toolbox_playlists");
        div.setAttribute("class", "playlist_toolbox playlist_toolbox_playlists");

        for (x = 0; x < playlist.length; x++) {
            var span = document.createElement("SPAN");
            span.textContent = playlist[x].name;
            span.setAttribute("onclick", `playlist.add("${playlist[x].uuid}", ${beatmap_id}, ${beatmap_set_id});`);
            div.append(span);
        }
    ...
    playlist_page.prototype.show_options = function(playlist_uuid, beatmap_set_id, beatmap_id, index) {
        ...
        function display_tooltip(e) {
            ...
            document.getElementById("playlist_toolbox").style.left = e.clientX + (e.clientX < 350 ? 170 : 0) + "px";
            posx = e.clientX + (e.clientX < 350 ? 170 : 0);
            posy = e.clientY;
        }
        ...
        function show_playlists() {
            document.getElementById("toolbox").append(that.toolbox_playlists(playlist.get(), beatmap_id, beatmap_set_id));
            document.getElementById("playlist_toolbox_playlists").style.top = posy + "px";
            document.getElementById("playlist_toolbox_playlists").style.left = posx + "px";
            document.getElementById("playlist_toolbox_playlists").style.transform = `translate(calc(-100% - 170px), -${document.getElementById('playlist_toolbox').clientHeight}px)`;
            document.getElementById("playlist_toolbox_playlists").addEventListener('click', remove_playlists);
            window.addEventListener('mouseover', remove_playlists_mouseoff);
        }

        function remove_playlists() {
            if (document.getElementById("playlist_toolbox_playlists")) {
                document.getElementById("playlist_toolbox_playlists").remove();
            }
        }

        function remove_playlists_mouseoff(e) {
            var container = document.getElementById("playlist_toolbox_playlists");
            var addplaylistbutton = document.getElementById("toolbox_add_to_playlist");
            if (container && addplaylistbutton) {
                if (container !== e.target && !container.contains(e.target) && addplaylistbutton !== e.target && !addplaylistbutton.contains(e.target)) {
                    window.removeEventListener('mouseover', remove_playlists_mouseoff); 
                    remove_playlists();
                }
            }
        }
        ...
        function remove() {
            ...
            if (document.getElementById("playlist_toolbox")) {
        ...
        setTimeout(function () {
            ...
            // If clicked outside options div remove
            window.addEventListener('click', remove_onclick);

            // If right clicked outside options div remove
            window.addEventListener("contextmenu", remove_onclick);

            // On resize windows remove
            window.addEventListener('resize', remove);

            // Remove listeners if these buttons are clicked.
            document.getElementById("toolbox_add_to_playlist").addEventListener('mouseover', show_playlists);
            document.getElementById("toolbox_view_osu").addEventListener('click', remove);
            document.getElementById("toolbox_download_beatconnect").addEventListener('click', remove);
            document.getElementById("toolbox_delete_song").addEventListener('click', remove);
            document.getElementById("toolbox_delete_song_set").addEventListener('click', remove);

            if (document.getElementById("toolbox_remove_from_playlist")) {
                document.getElementById("toolbox_remove_from_playlist").addEventListener('click', remove);
            }
        ...
        if (name_playlist) {
            ...
            function addcard(cover, title, artist, duration, source, created, name_playlist, BeatmapSetID, BeatmapID) {
                setTimeout(() => {
                    ...
                    setTimeout(() => {
                        card = document.getElementById(`card_${name_playlist}_${BeatmapID}`);
