...
async function load(page_Playlist, playlist_name){
    page_playlist_length = Object.keys(page_Playlist).length;
    ...
    for(i = 0; i < page_playlist_length; i++) {
        ...
        beatmap_id = page_Playlist[i]['BeatmapSetID'];
        ...
        song_folder = path.join(__dirname, "songs", beatmap_id);
        song_path = path.join(song_folder, file);

        duration = format_seconds(page_Playlist[i]['length']);

        created = await data_added(song_folder);

        cover = path.join(__dirname, "songs", beatmap_id, "cover.jpg");
        cover = cover.replace(/\\/g, '/');
        card = playlist_card(cover, title, artist, duration, source, created, playlist_name, playlist_name, i);
    ...
    try { updatePlaylistIcon(); } catch (err) {}
}
...
function playlist_card(cover, title, artist, duration, source, created, playlist_name, playlist_name, id) {
    ...
    span_pb.setAttribute("onclick", `play_from_playlist(${i}, '${playlist_name}')`);
