...
const playlist = function() {
    ...
    playlist.prototype.create = async function(name, description, cover) {
        let data = await parse_json.read_file(collection);
        if (!data) {
            data = {}
        } else {
            data = JSON.parse(data)
        }
        let used_uuidv4 = Object.keys(data);
        ...
        data[utils.uuidv4()] = { name: name, description: description, cover: cover, created: Date.now(), songs: [] };
        this.write(data);
        console.log(data);
    }

    playlist.prototype.get = async function(playlist_uuid) {
        let data = JSON.parse(await parse_json.read_file(collection));
        if (playlist_uuid) {
            if (Object.keys(data).includes(playlist_uuid)) {
                return data[playlist_uuid]
            } else {
    ...
    playlist.prototype.add = async function(playlist_uuid, beatmapset_id, beatmap_id) {
        let data = JSON.parse(await parse_json.read_file(collection));
        if (Object.keys(data).includes(playlist_uuid)) {
            data[playlist_uuid]["songs"].push({BeatmapID: beatmap_id, BeatmapSetID: beatmapset_id})
            this.write(data);
        } else {
            throw "Playlist does not exist"
        }
    ...
    playlist.prototype.write = function(data) {
        fs.writeFile(collection, JSON.stringify(data), function(err) {
            if (err) {
                console.log(err);
            }
        });
    ...
    let collection = path.join(appDataPlaylists, "playlist.json");
