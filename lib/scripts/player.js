const Player = function() {
    ...
    Player.prototype.getCurrentPlaylistName = function() {
    ...
    Player.prototype.build = async function() {
        ...
        if (current_playlist.length > 0) {
            ...
            this.initPlayer(current_playlist[0]);
    ...
    Player.prototype.play_from_playlist = async function(beatmap_id, playlist_name) {
        if (currentPlaylistName == playlist_name) {
            ...
            for (var x = 0; x < playlist.length; x++) {
                var a = currentPlaylistName == "Library" ? playlist[x].BeatmapID : playlist[x].index;
                if (current) {
                    var b = currentPlaylistName == "Library" ? current.BeatmapID : current["index"];
                }
    ...
    Player.prototype.backward = function() {
        if (current && current_playlist || shuffled_playlist) {
            ...
            for(var i = 0; i < playlist.length; i++) {
                var a = currentPlaylistName == "Library" ? playlist[i].BeatmapID : playlist[i].index;
                var b = currentPlaylistName == "Library" ? current.BeatmapID : current.index;
                if (a == b) {
    ...
    Player.prototype.shuffle_playlist = function() {
        if (current_playlist) {
            ...
            for (var i = 0; i < shuffled_playlist.length; i++) {
                var a = currentPlaylistName == "Library" ? current.BeatmapID : current.index;
                var b = currentPlaylistName == "Library" ? shuffled_playlist[i].BeatmapID : shuffled_playlist[i].index;
                if (a == b) {
    ...
    Player.prototype.player = function(data, playlist) {
        ...
        if (current && data) {
            var a = currentPlaylistName == "Library" ? data.BeatmapID : data["index"]
            var b = currentPlaylistName == "Library" ? current.BeatmapID : current["index"];
        }
        console.log(playlist)
        console.log(currentPlaylistName)
        if (current && a == b && currentPlaylistName == playlist) {
            if (audio) {
                if (audio.playing()) {
                    audio.pause();
                    console.log("Pause")
    ...
    Player.prototype.initPlayer = function(data) {
        ...
        duration_interval = setInterval(() => this.updateTimer(), 1000);
        // Howler Events
        audio.on("end", () => {
            this.forward();
            this.updatePlaylistIcon('end');
    ...
    Player.prototype.resetTimerAndSlider = function() {
    ...
    Player.prototype.updatePlaylistIcon = function(state) {
        if (audio) {
            ...
            var a = currentPlaylistName == "Library" ? current.BeatmapID : current.index;
            playlist_icon = `play_${currentPlaylistName.replace(/\s+/g, '_')}_${a}`;
            playlist_card = `card_${currentPlaylistName.replace(/\s+/g, '_')}_${a}`;
