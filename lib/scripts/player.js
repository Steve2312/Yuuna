...
class Player {
    ...
    async play_from_playlist(beatmap_id, playlist_name) {
        ...
        var playlist = this.#shuffle ? this.#shuffled_playlist : this.#current_playlist;
        for (var x = 0; x < playlist.length; x++) {
            if (playlist[x].BeatmapID == beatmap_id) {
                this.#player(playlist[x], playlist_name);
            }
        }
        if (this.#shuffle) this.#shuffle_playlist();
    }
    ...
    async updatePlaylist() {
        console.time("Update the shuffle playlist: ");
        ...
        if (LOADED_PAGE == "playlist_page") {
            setTimeout(function () {
                Libraryload_Playlist(that.#currentPlaylistName);
            }, 500);
        }
        console.timeEnd("Update the shuffle playlist: ");
    }
    ...
    #createSongPath = (BeatmapSetID, file) => {
        return path.join(appDataSongs, BeatmapSetID, file);
    }
    ...
    #initPlayer = (data) => {
        ...
        this.#audio = new Howl({
            src: [this.#createSongPath(data.BeatmapSetID, data.file)],
            html5: true,
            volume: this.#audio_volume
        });
    ...
    async #updateAudioData() {
        ...
        this.#setRPC('paused');
        var coverPath = path.join(appDataSongs, BeatmapSetID, "cover.jpg").replace(/\\/g, '/');
