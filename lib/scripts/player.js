const Player = function() {
    ...
    Player.prototype.build = async function() {
        current_playlist = await request_JSON(currentPlaylistName);
    ...
    Player.prototype.play_from_playlist = async function(beatmap_id, playlist_name) {
        ...
        current_playlist = await request_JSON(playlist_name);
    ...
    Player.prototype.updatePlaylist = async function() {
        ...
        if (LOADED_PAGE == "playlist_page") {
            setTimeout(() => playlist_page.Libraryload_Playlist(currentPlaylistName), 500);
        }
    }
    ...
    Player.prototype.change_volume = function() {
        ...
        if (preview_audio) {
            preview_audio.volume(this.audio_volume);
        }
    }
